...
export async function sendSalesInquiryEmail(data: SalesIntakeValues) {
  ...
  const entries: Array<[string, string | undefined | null]> = [
    ...
    ['Engagement', data.engagement_model],
    ['Tech stack', techStack],
    ['Services', services],
    ['Goals', data.goals],
    ['Target start', data.start_date],
...
<html>
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" style="max-width:560px;width:100%;margin:0 auto;border-collapse:collapse;">
      <tr>
        <td style="padding:0 0 16px 0;">
          <p style="margin:0;color:#3B82F6;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.18em;">New sales inquiry</p>
          <h1 style="margin:8px 0 0 0;color:#0F172A;font-size:22px;font-weight:700;line-height:1.3;">${escapeHtml(data.full_name)} — ${escapeHtml(data.project_type)}</h1>
        </td>
      </tr>
      <tr>
        <td>
      ...
      <tr>
        <td style="padding:24px 0 0 0;color:#94A3B8;font-size:12px;">
          Reply directly to this email to reach ${escapeHtml(data.full_name)}.
        </td>
      </tr>
    </table>
  </body>
  ...
  return resend().emails.send({
    ...
    subject: `New sales inquiry — ${data.full_name} (${data.project_type})`,
    html,
  });
}
